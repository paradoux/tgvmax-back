...
function requireAllModels(Implementation, modelsDir) {
  if (modelsDir) {
    return readdirAsync(modelsDir).each(function (file) {
      try {
...
exports.init = function (Implementation) {
  ...
  requireAllModels(Implementation, absModelDirs).then(function (models) {
    integrator = new Integrator(opts, Implementation);

    return Schemas.perform(Implementation, integrator, models, opts).then(function () {
      var directorySmartImplementation = void 0;
    ...
    var resourcesRoute = new ResourcesRoutes(app, model, Implementation, integrator, opts);
    ...
    return new ForestRoutes(app, opts).perform();
  }).then(function () {
    return app.use(errorHandler.catchIfAny);
  }).then(function () {
    if (opts.envSecret && opts.envSecret.length !== 64) {
      ...
      var apimap = new JSONAPISerializer('collections', collections, {
        ...
        attributes: ['name', 'nameOld', 'displayName', 'paginationType', 'icon', 'fields', 'actions', 'segments', 'onlyForRelationships', 'isVirtual', 'integration', 'isReadOnly', 'isSearchable'],
        fields: {
          attributes: ['field', 'displayName', 'type', 'relationship', 'enums', 'collection_name', 'reference', 'column', 'isFilterable', 'widget', 'integration', 'isReadOnly', 'isVirtual', 'isRequired', 'defaultValue', 'validations', 'isSortable']
        },
        validations: {
        ...
        actions: {
          ref: 'id',
          attributes: ['name', 'baseUrl', 'endpoint', 'redirect', 'download', 'global', 'type', 'httpMethod', 'fields'] // TODO: Remove global attribute when we remove the deprecation warning.
        },
        segments: {
          ref: 'id',
          attributes: ['name']
    ...
    return ipWhitelist.retrieve(opts.envSecret).catch(function (error) {
      return logger.error(error);
    });
  }).catch(function (error) {
    logger.error('An error occured while computing the Forest apimap. Your ' + 'application apimap cannot be sent to Forest. Your Admin UI might ' + 'not reflect your application models.', error);
  });
