...
module.exports = function routes(app, model, Implementation, opts) {
  ...
  var getSubscriptions = function getSubscriptions(request, response, next) {
    new SubscriptionsGetter(Implementation, _.extend(request.query, request.params), opts, integrationInfo).perform().then(function (results) {
      ...
      return new SubscriptionsSerializer(subscriptions, modelName, { count: count });
  ...
  var getBankAccounts = function getBankAccounts(request, response, next) {
    ...
    new SourcesGetter(Implementation, _.extend(request.query, request.params), opts, integrationInfo).perform().then(function (results) {
      ...
      return new BankAccountsSerializer(bankAccounts, modelName, { count: count });
  ...
  this.perform = function () {
    if (integrationInfo) {
      app.get(path.generate(modelName + '_stripe_payments', opts), auth.ensureAuthenticated, getPayments);

      app.get(path.generate(modelName + '_stripe_payments/:paymentId', opts), auth.ensureAuthenticated, getPayment);

      app.get(path.generate(modelName + '/:recordId/stripe_payments', opts), auth.ensureAuthenticated, getPayments);

      app.post(path.generate(modelName + '_stripe_payments/refunds', opts), auth.ensureAuthenticated, refund);

      app.get(path.generate(modelName + '_stripe_invoices', opts), auth.ensureAuthenticated, getInvoices);

      app.get(path.generate(modelName + '/:recordId/stripe_invoices', opts), auth.ensureAuthenticated, getInvoices);

      app.get(path.generate(modelName + '_stripe_invoices/:invoiceId', opts), auth.ensureAuthenticated, getInvoice);

      app.get(path.generate(modelName + '/:recordId/stripe_cards', opts), auth.ensureAuthenticated, getCards);

      app.get(path.generate(modelName + '_stripe_cards', opts), auth.ensureAuthenticated, getCard);

      app.get(path.generate(modelName + '_stripe_subscriptions', opts), auth.ensureAuthenticated, getSubscriptions);

      app.get(path.generate(modelName + '_stripe_subscriptions/:subscriptionId', opts), auth.ensureAuthenticated, getSubscription);

      app.get(path.generate(modelName + '/:recordId/stripe_subscriptions', opts), auth.ensureAuthenticated, getSubscriptions);

      app.get(path.generate(modelName + '/:recordId/stripe_bank_accounts', opts), auth.ensureAuthenticated, getBankAccounts);

      app.get(path.generate(modelName + '_stripe_bank_accounts', opts), auth.ensureAuthenticated, getBankAccount);
    }
  };
};