...
module.exports = function (app, model, Implementation, options) {
  ...
  this.listConversations = function (request, response, next) {
    new ConversationsGetter(Implementation, _.extend(request.query, request.params), options, integrationInfo).perform().spread(function (count, conversations) {
      return new ConversationsSerializer(conversations, modelName, { count: count });
  ...
  this.perform = function () {
    if (integrationInfo) {
      app.get(path.generate(modelName + '/:recordId/intercom_attributes', options), auth.ensureAuthenticated, this.getAttributes);
      app.get(path.generate(modelName + '/:recordId/intercom_conversations', options), auth.ensureAuthenticated, this.listConversations);
      app.get(path.generate(modelName + '_intercom_conversations/:conversationId', options), auth.ensureAuthenticated, this.getConversation);
    }
  };
};