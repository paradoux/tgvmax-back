...
module.exports = function (app, model, Implementation, integrator, opts) {
  ...
  function getAssociation(request) {
    ...
    return { associationName: associationName };
  }
  ...
  function getContext(request) {
    ...
    return { params: params, associationModel: associationModel };
  }
  ...
  function list(request, response, next) {
    ...
    return new Implementation.HasManyGetter(model, associationModel, opts, params).perform().then(function (results) {
      var records = results[0];
      var fieldsSearched = results[1];

      return new ResourceSerializer(Implementation, associationModel, records, integrator, opts, null, fieldsSearched, params.search).perform();
  ...
  function count(request, response, next) {
    ...
    return new Implementation.HasManyGetter(model, associationModel, opts, params).count().then(function (count) {
  ...
  function exportCSV(request, response, next) {
    ...
    var recordsExporter = new Implementation.RecordsExporter(model, opts, params, associationModel);
  ...
  this.perform = function () {
    ...
    _.each(SchemaUtil.getHasManyAssociations(schema), function (association) {
      app.get(path.generate(modelName + '/:recordId/relationships/' + association.field + '.csv', opts), auth.ensureAuthenticated, exportCSV);
      app.get(path.generate(modelName + '/:recordId/relationships/' + association.field, opts), auth.ensureAuthenticated, list);
      app.get(path.generate(modelName + '/:recordId/relationships/' + association.field + '/count', opts), auth.ensureAuthenticated, count);
      app.post(path.generate(modelName + '/:recordId/relationships/' + association.field, opts), auth.ensureAuthenticated, add);
      // NOTICE: This route only works for embedded has many
      app.put(path.generate(modelName + '/:recordId/relationships/' + association.field + '/:recordIndex', opts), auth.ensureAuthenticated, updateEmbeddedDocument(association.field));
      app.delete(path.generate(modelName + '/:recordId/relationships/' + association.field, opts), auth.ensureAuthenticated, remove);
    });

    // NOTICE: belongsTo associations routes
    _.each(SchemaUtil.getBelongsToAssociations(schema), function (association) {
      app.put(path.generate(modelName + '/:recordId/relationships/' + association.field, opts), auth.ensureAuthenticated, update);
    });
  };
};