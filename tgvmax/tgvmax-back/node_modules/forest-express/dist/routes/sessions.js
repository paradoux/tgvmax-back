...
module.exports = function (app, opts, dependencies) {
  ...
  function loginWithPassword(request, response) {
    ...
    return ipWhitelist.retrieve(envSecret).then(function () {
      return new AllowedUsersFinder(renderingId, envSecret).perform();
      ...
      return bcrypt.compare(request.body.password, user.password).then(function (isEqual) {
        if (!isEqual) {
          throw new Error();
        }
        return user;
      });
    }).then(sendToken(response, renderingId)).catch(formatAndSendError(response));
  }
  ...
  function loginWithGoogle(request, response) {
    ...
    ipWhitelist.retrieve(envSecret).then(function () {
